...
of critical points in the input mesh, which could potentially be much smaller than $n$.  In particular, 
Cole-McLaughlin and Pascucci \cite{pc-ectls-02} provide an $O(n+t\log n)$ time algorithm for the case of 
$3$ dimensional structured meshes.  \Ben{Update distinction between critical and component-critical}
Chiang \etal \cite{cllr-sooscctmp-05} improve upon this result, and provide an $O(N+t\log t)$ algorithm 
which works for structured or unstructured meshes in any dimension, which is provably optimal with respect 
to the output size but not the output structure, as discussed below.
...
\begin{figure}[h]\centering
    \includegraphics[width=.18\linewidth]{figs/orderGray1}%
    \hspace{.8in}
    \includegraphics[width=.18\linewidth]{figs/orderGray2}
    \caption{On left, downward paint spilling only (each shade of gray represents a different color).  On right, flipping the direction of paint spilling.}
    \label{fig:order}
\end{figure}

\section{Contour tree basics} \label{sec:basics}

We detail the basic definitions about contour trees. Most of this is taken from Chapter 6 of Carr's thesis~\cite{Carr}.
Our input is a continuous PL function $f:\MM \mapsto \RR$, where $\MM$ is a simplicial complex in $\RR^d$. 
All simplices are isomorphic to a standard $d$-dimensional simplex, except for specially designated
\emph{boundary} simplices. \Sesh{Some mention of representation and data structures here.}
It is standard to assume
that $f$ is Morse (we explain later what this entails). 
...
Not that a contour is itself a simplicial complex of one dimension lower, and is represented (in our algorithms) as such.
...
neighbors of $F$ not in $F$ either have function values strictly greater or strictly less
than this value.
\end{definition}
...
represented as $d-1$-dimensional simplicial complices.
...
there is an edge $(v_i,v_j)$. (Conventially, edges are directed from higher function value to lower.)
\end{definition}

(It is not immediately obvious that this graph is a tree. But it is equivalent to the Reeb graph
of $f:\MM \rightarrow \RR$, which is known to be a tree.) We denote the contour tree of $\MM$
by $\reeb(\MM,f)$. The corresponding node and edge sets are denoted as $\cV(\cdot)$ and $\cE(\cdot)$.

We use a key theorem from~\cite{Carr} (Theorem 6.6) that maps paths in $\cC(\MM)$ and $\MM$.

\begin{theorem} \label{thm:carr-path} For every path $P$ in $\MM$, there exists a path $Q$ in the contour tree to which $P$
maps, and for every path $Q$ in the contour tree, there exists at least one path $P$ in $\MM$ that maps to $Q$.
\end{theorem}
...
An $h$-contour is achieved by intersecting $\MM$ with the hyperplane $x_{d+1} = h$. (Think of the $d+1$-dimension
...
\begin{lemma} \label{lem:cut} Consider a regular contour $\phi$ contain in contour class (edge of $\reeb(\MM))$
$(u,v)$ and denote $\MM' = \cut(\phi,\MM)$. Then $\cV(\reeb(\MM')) = \{\phi^+,\phi^-\} \cup (\cV(\MM) \setminus \phi)$
...
all the edges of this path are in $\cE(\cC(\MM)) \setminus (u,v)$. So we get a cycle in $\cC(\MM)$. Contradiction.
...
a minimum $x$ such that every non-minimal \emph{vertex} in the manifold has decreasing path to $x$.
...
is denoted by $\wet(x,\MM)$ (which is in turn represented as a simplicial complex). A point $z$ is at the \emph{interface} of $\wet(x,\MM)$ if every neighborhood of $z$
...
\begin{proof} If $y \in \wet(x,\MM)$, all the points in any contour containing $y$ are also in $\wet(x,\MM)$.
 (Follow the non-ascending path to from $x$ to $y$ and then walk along the contour.) The converse is also true,
...
There must be a dry point in $B_\eps(y)$ that is above $y$, and hence,
there exists a dry, regular $(f(y)+\delta)$-contour $\phi$ intersecting $B_\eps(y)$.

Suppose $y$ is in the interface. Consider the set $\Gamma_y$ of points connected to $y$
by a path with all points at height $f(y)$. (This is a set of $f(y)$-contours.) Suppose for contradiction
that $\forall z \in \Gamma_y$, $z$ has up-degree $1$. Consider the non-ascending path from $x$ to $y$ and let $z$
...
have up-degree $1$ (ref. to \Def{deg}), then $\psi$ is the unique $f(y)+\delta$-contour
intersecting $B_\eps(w)$. This would imply that $\phi = \psi$, contradicting the fact that $\phi$ is wet
and $\psi$ is dry.
...
lies in $\Gamma_y$ (which is wet). Hence, this gives the interface, which is a contour containing
the join $w$.
\end{proof}
...
We perform a non-ascending walk from $x$. Any facet $F$ of $\MM$ encountered has some (if not all) portion of in $\wet(x,\MM)$.
...
$f(y) + \delta$. Let $\lift(\phi)$ be the unique contour through regular point $z$. Note that $\lift(\phi)$ is dry.
...
of the procedure is that the rain alternately flows downwards and upwards. In other words, when
we start, rain flows downwards. In each recursive call, the direction of rain is switched to the 
opposite direction. While this is way of implementing $\rain$, it is conceptually easier
...
A facet $F \in \MM$ is cut at most once during the $\rain(x,\MM)$.
\end{lemma}
\begin{proof} All notation here follows that in the pseudocode of $\rain$.
Firstly, by \Thm{jordan}, all the pieces of which $\rain$ is invoked are disjoint.
Secondly, all recursive calls are made on dry complices.

Consider the first time that $F$ is cut, say, during the call to $\rain(x,\NN)$.
Specially, say this happens when $\cut(\phi'_i,\NN_i)$ is constructed. 
...
complexities of the wet complices and the new boundaries created. No facet is every wet twice,
...
We observe that the contours trees of minimum dominant manifolds have a relatively simple description.
This description is directly inspired by (and proven using) the \emph{join tree} of~\cite{csa-cctad-00}.
We first explain what these trees are.

For vertex $v$, we use $\MM^+_v$ to denote the simplicial complex obtained by only
keeping vertices $u$ such that $f(u) > f(v)$. Analogously, define $\MM^-_v$. Note that $\MM^+$ may contain
numerous connected components. The following definition of join and split trees were given by~\cite{csa-cctad-00}.
...
The directed edge $(u,v)$ is present when $u$ is the smallest valued vertex is a connected component of $\MM^+_v$
...
Some basic facts about these trees. All outdegrees in $\cJ(\MM)$ and all indegrees in $\cS(\MM)$ are at most $1$.
...
\begin{proof} First, let us complete the proof assuming that each split $v$ has one child that is just a leaf, which
is a non-dominant minimum. Any minimum is a leaf in $\cS(\MM)$, and thereby attached to a split. If we removed
all non-dominant minima, we must end up with a path, as asserted above.
...
on this path. It must intersect some contour, say $\phi$. By \Thm{jordan}, $\cut(\MM,\phi)$ has
two connected components, with one (call it $\LL$) having $\phi^-$ as a boundary maximum. This complex
contains $m^*$ as the non-ascending path $\phi$ intersects $\phi$ only once. Let the other
...
It is convenient to denote the non-dominant minima by $m_1, m_2, \ldots, m_k$, and the associated
splits (as given by \Lem{split}) by $s_1, s_2, \ldots, s_k$.
...
For any tree $T$ with vertex $v$, we define \emph{erasing} $v$ from $T$,
when both in and outdegrees are at most $1$. If $v$ is a leaf, just delete $v$. Otherwise, smooth $v$ out
...
\begin{theorem} \label{thm:contour-tree} The contour tree $\cC(\MM)$ consists of all edges $\{(s_i, m_i)\}$
as well as: for any critical point $v$, edge $(w,v)$ where $w$ is the lowest non-minimal critical point
in a connected component of $\MM^+_v$ connected to $v$.
\end{theorem}
...
Now we come to the main algorithmic contribution. We design a new algorithm for computing contour
trees of a minimum dominant simplicial complice $\MM$. As part of the initialization, we determine
...
of \emph{painting} of $\MM$. Initially associate a color with each maximum. Denoting the set of maxima is $X$,
this is an arbitrary bijection $\chi:X \mapsto [|X|]$. Imagine there being a large
can of paint of color $\chi(x)$ at maxima $x$. We will spill different paint from each maximum and watch it flow down.
...
do not mix, so eventually, every vertex and edge of $\MM$ gets a unique color. This process (and indeed the entire algorithm)
works purely on the 1-skeleton of $\MM$, which is just a graph.
%
\begin{definition} \label{def:paint} Let the 1-skeleton of $\MM$ be denoted $G(\MM) = (\cV,E)$.
A  \emph{painting} of $\MM$ is a map $\chi:X \cup E \mapsto [|X|]$ with the following properties. 
\begin{asparaitem}
	\item The restriction $\chi:X \mapsto [|X|]$ is a bijection.
...
\end{definition}
%
Note that paintings are not unique. It is straightforward to construct a painting of $\MM$ in $O(|\MM|)$ time, using a descending BFS from each maximum that does not explore previously colored edges. 
...
\begin{definition} \label{def:color-full} Consider vertex $v$, and let $\phi$ be a $(f(v)+\delta)$-contour in
...
Note that $\fcol(v)$ has size at most $2$, by the Morse condition. Note that $\col(v)$ and $\fcol(v)$ can
...
\medskip
\noindent
{\bf The binomial heaps $\touch(c)$:} For each color $c$, $T(c) = \{v | c \in \fcol(v)\}$.
This is stored as a \emph{binomial max-heap} $T(c)$, keyed by vertex heights.
...
\subsection{Properties of a painted simplicial complex} \label{sec:prop}

\begin{lemma} \label{lem:full} Consider vertex $v$ and take $(f(v)+\delta)$-contour $\phi$ intersecting
...
\begin{proof} Consider the highest
...
\begin{definition} \label{def:ripe} A vertex $v$ is \emph{ripe} if: for all $c \in \fcol(v)$, $v$
is the highest vertex in $T(c)$. Also, for all $c \in \col(v) \setminus \fcol(v)$, $T(c)$ is empty.
\end{definition} 
...
\begin{compactenum}
	...
	\begin{compactenum}
		\item (Either (i) there is $c \in \fcol(v)$ such that $v$ is not the highest in $T(c)$ or (ii) there is 
		$c \in \col(v) \setminus \fcol(v)$, such that $T(c)$ is non-empty.)
		\item In either case, push the highest of $T(c)$ onto $\st$, and update head $h$.
	\end{compactenum}
\end{compactenum}
\end{minipage}}

\medskip
Clearly, when $\update(\st)$ terminates, the head $h$ is ripe. The following crucial claim demonstrates
the utility of the stack.

\begin{claim} \label{clm:stack} Vertices are always pushed onto $\st$ in increasing order of height.
\end{claim}

\begin{proof} Consider a single push onto the stack. In Case (i), the increase in height is obvious.
