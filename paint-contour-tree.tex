...
\author{
  ...
  \and
  C. Seshadhri\thanks{Sandia National Laboratories; Livermore, CA, USA; {\tt scomand@sandia.gov}}
}
...
a refined lower bound matching the performance of \Thm{main-alg}. 
...
Consider the set of functions $\bF_C$ that have the same number of critical points and the same (valid) cost $C$.  Any algebraic decision tree that correctly computes
...
Trees that are short and fast are easy to compute, whereas trees with long paths are hard.
Our result is related to the concept of instance optimal algorithms, as defined by Afshani \etal \cite{abc-ioga-09}.
...
\begin{figure}[h]\centering
    ...
    \caption{On left, downward rain spilling only (each shade of gray represents a different color), producing a grid.  
...
We thank Jeff Erickson and Yusu Wang for numerous useful discussions. This work is
...
\label{sec:lb}
In this section we provide a lower bound for the time it takes to compute a contour tree in terms the cost of path decompositions.  
