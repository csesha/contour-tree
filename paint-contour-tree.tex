...
(where $\alpha(\cdot)$ is the inverse Ackermann function). A further improvement to $O(t\log t + N\alpha(N))$ time
...
our algorithm runs in $O(\sum_{p\in P} |p|\log|p| + N\alpha(N))$. If the contour tree is a balanced
binary tree, our running time is $O(n + N\alpha(N))$. Moreover, we prove a lower bound of
exactly $\Omega(\sum_{p \in P} |p|\log |p|)$ for computing contour trees, thereby showing
...
A large class of data, such as satellite terrain maps, physical simulations, combustion data, can
be thought as a function $f: \RR^d \mapsto \RR$. Typically, a finite representation is given
by consider $f$ to be piecewise linear over some mesh (simplicial complex) $\MM$ in $\RR^d$.
...
exactly where the topology of the contours change. The vertices of the contour tree are exactly the critical points.
...
and studying chemical systems~\cite{LaBe+06,BrWe+10,BeWe+11,BrWe+11,MaGr+11}.
...
by edge $e$ of color $c$. Either all edges in $\PP$ are colored $c$, or there
...
\begin{proof}
 ...
 adjacent color $c$.  Therefore, $h$ must be in the current heap of $\rep(c)$ (since elements are only removed 
 from heaps when being processed).  Now by \Lem{full}, either all edges in $\PP$ are colored $\rep(c)$ or  
 ...
 implying that $h$ is not the highest vertex.  Otherwise, all the vertices in $\PP$ have been processed.  
 However, it cannot be the case that all vertices in all components of $\MM^+_h$ have already been processed, 
 since this would imply that $h$ was ripe and so $h$ so can apply the same argument to the other non-fully processed component. 
 ...
 and so it must be in $T(\rep(c))$ since it must be in some heap of a color in each upstar (and one upstar is entirely colored $\rep(c)$).
...
are edges of $\jc(\MM)$ and for any color $c$, $\h(c)$ is the lowest processed vertex
...
as $v$ is the lowest processed vertex of this merged color (as by induction $\h(c)$ and $\h(c')$ where the lowest vertices before merging colors).
