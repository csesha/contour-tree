...
\begin{lemma}
\label{lem:adjacent}
 Let $p$ be any path in $P(T)$ and let $\{q_1, \dots q_k\}$ be the children of $p$.
...
\subsection{Pruning Away Buffered Sub-shrubs}
Let $\cU$ be a shrub in $\cL \setminus \cL'$.  For any node $q$ in the shrub $\cU$, let $TSC(q)$ denote the set of all vertices of $T$ from all paths in the tall support chain of $q$.
...
% In the following, any connected subgraph of a shrub will be called a \emph{sub-shrub} (i.e. it may not be a proper subtree).
% 
% \begin{definition}
%  Let $\cU$ denote a shrub of $\cL \setminus \cL'$.  
%  Then a sub-shrub $\buff$ of $\cU$ is called \emph{buffered} if the following holds:
%  \begin{enumerate}
%   \item $\sum_{p\in \buff} \loss(p) \geq \frac{1}{100} \sum_{p\in \buff} |p|$
%   \item $\buff$ is minimal, i.e.\ there is no sub-shrub of $\buff$ for which the above inequality holds.  
%  \end{enumerate}
% \end{definition}
...
\begin{definition}
  Let $\cU$ denote a shrub of $\cL \setminus \cL'$. A \emph{pruning} $\cU'$ of $\cU$ is any subgraph defined by some number of rounds of the following iterative process.  Initially set $\cU' = \cU$.  In each iteration select some subtree $\cU'_r$ rooted at a node $r\in \cU'$ and set $\cU' = \cU'\setminus \{\cU'_r\}$.  
\end{definition}
...
\begin{definition}\label{def:buff}
 Let $\cU'$ be a pruning of a shrub of $\cU$ of $\cL \setminus \cL'$. Then a rooted subtree $\buff$ of $\cU'$ is called a \emph{buffered sub-shrub} if the following conditions hold:
  \begin{enumerate}
  \item $\sum_{p\in \buff} |\loss(p)| \geq \frac{1}{100} \sum_{p\in \buff} |p|$
...
% \begin{definition}
%  Let $\cU$ denote a shrub of $\cL \setminus \cL'$. We define an interative  procedure which ``prunes'' $\cU$.  Specifically, let $\cU'$ denote the current remaining portion of the shrub, so initially, $\cU' = \cU$.  A single iteration consists of the following: Visit the nodes in $\cU'$ in a reverse topological order (i.e. leaves first).  Let $q$ be the current node being considered, and let $\cU'_q$ be the subtree rooted at $q$.  We say $\cU'_q$ is buffered if the following condition holds
%  \[
%   \sum_{p\in \cU'_q} \loss(p) \geq \frac{1}{100} \sum_{p\in \cU'_q} |p|
%  \]
%  An iteration stops when we reach a node $q$ defining a buffered subtree, at which point we set $\cU' = \cU'\setminus{\cU'_q}$.
% \end{definition}

Ultimately we will prune buffered subtrees from $\cU$ by starting at the leaves and working towards the root, and hence the second condition of buffered will trivially be satisfied. 

We require the following lemma so that we can charge to the root in a buffered subtree.


\begin{lemma}
 Let $p$ be any node in a buffered sub-shrub $\buff$, and let $d$ be any descendant node of $p$ in $\buff$.  Then for any $v \in d$, $|H_v| = O(|H_p|)$.
\end{lemma}
\begin{proof}
 Missing.  \Lem{cu-root} will be rephrased to handle this slightly more general case (i.e. here $p$ may not be the root of $\cU$). Note the proof will not 
 have to be changed as \Lem{geometric} is the proof which already implies general case.
\end{proof}
...
\end{lemma}
\begin{proof}
 ????  Ignore this ``proof'', it is just a graveyard of observations. ????
...
Let $p$ and $q$ be two distinct paths in $\cL \setminus \cL'$.  Note that while it is possible that $TSC(p)\cap TSC(q)\neq \emptyset$, 
...
\begin{corollary}
 Let $B= \{\buff_1, \ldots, \buff_k\}$ be any collection of disjoint buffered sub-shrubs from $\cL \setminus \cL'$. Then, 
...
The second term can be bounded by $O(\sum_{p \in \cL'} |H_p| \log |H_p|)$, again by \Lem{pathBounds}.
The following claim shows that this in turn is asymptotically bounded by the last term.
...
By \Lem{cu-root}, $|H_v| = O(|H_r|)$, where $r$ is the root of $\cU$, and furthermore, $\sum_{p \in \cU} |p| = O(|H_r|)$. 
