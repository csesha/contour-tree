...
our algorithm runs in $O(\sum_{p\in P} |p|\log|p| + t\alpha(t)+N)$, which is 
always bounded by $O(t\log t + N)$ and is $O(N+t\alpha(t))$ for balanced trees. 
Moreover, we prove a general lower bound of
...
by considering $f$ to be piecewise linear over some mesh (i.e. simplicial complex) $\MM$ in $\RR^d$.
...
$\MM$ in $\RR^{d+1}$ with the last coordinate (i.e. height) given by $f$. Imagine sweeping the hyperplane $x_{d+1} = h$
...
\Fig{relative} and \Fig{sorting} show examples of simplicial complexes, with heights and their contour trees.
...
hence must pay $\Omega(t\log t)$ for all instances. Can we beat this sorting bound? Is there a more refined version of the complexity
...
%A leaf path runs either upwards or downwards, never both.
%
...
\begin{theorem} \label{thm:main-alg} Consider a PL-Morse\footnote{This is a standard ``well-behaved" assumption that implies
...
A better but still obvious upper bound is $O(t\log D + t\alpha(t) + N)$, where $D$ is the diameter of $P(T)$. 
...
We complement \Thm{main-alg} with an almost matching lower bound, showing that the complexity
of $\sum_{p \in P(T)} |p|\log |p|$ is required for any contour tree algorithm. The exact phrasing
of the lower bound requires some care. Consider the $d=2$ (terrain) case, and fix a mesh $\MM$.
As referenced in \Thm{main-alg}, for any height function $f$ our algorithm defines a 
specific path decomposition $P(T)$\footnote{The specific decomposition used is defined in \Sec{pathDecomp}, 
but also depends on the raining and painting procedures described in \Sec{rain} and \Sec{paint}.  
The important thing is that our algorithm is deterministic, and so for a fixed $\MM$ and $f$ it is well defined.}. 
We define $\cost(f) = \sum_{p \in P(T)} |p|\log |p|$ for this specific decomposition $P(T)$.
Ideally, we would like to argue that any contour-tree algorithm
must pay $\cost(f)$ for $f$. 
But this is obviously false, since one could consider an algorithm
that hard codes the answer for $f$ and merely checks this answer.

Instead, call a possible cost $C$ \emph{valid} if $C = \cost(f)$ for some $f$ 
defined over the fixed mesh $\MM$\footnote{If one is concerned
that $C$ might be irrational, define valid if $2^C = 2^{\cost(f)}$.}. We prove
a refined lower bound matching \Thm{main-alg}. 

\begin{definition}\label{def:hfdt}
A \emph{height function decision tree} is an algorithm
that is allowed arbitrary computation on the domain $\MM$, and is an algebraic decision
...
(defined over a fixed mesh $\MM$).  Any height function decision tree that correctly computes
...
% of $\sum_{p \in P(T)} |p|\log |p|$ is required for any contour tree algorithm. The exact phrasing
% of the lower bound requires some care. Consider the $d=2$ (terrain) case, and fix a mesh $\MM$.
% Now any height function $f$ (together with $\MM$) defines a specific contour tree $T$.
% As referenced in \Thm{main-alg}, given $f$, our algorithm then defines some specific path 
% decomposition $\overline{P}(T)$, such that the running time of our algorithm is bounded by 
% $O(\cost(\overline{P}(T))+t\alpha(t)+N)$, where $\cost(P(T)) = \sum_{p \in P(T)} |p|\log |p|$.
% A priori, which path decomposition this will be is unclear\footnote{The specific decomposition, defined in \Sec{pathDecomp}, 
% depends on arbitrary choices made during the raining and painting procedures described in \Sec{rain} and \Sec{paint}.}, 
% however we know there is some large set of potential path decompositions it lies in
% (the exact set is defined in \Def{path2}, but for now one can think of the set of all decompositions).
% Let $\costs(f)$ denote the set of costs of all these potential path decompositions that $f$ defines
% (i.e. $\cost(\overline{P}(T))\in \costs(f)$).
% 
% Ideally, we would like to argue that any contour-tree algorithm must pay $\cost(\overline{P}(T))$ for $f$. 
% But this is obviously false, since one could consider an algorithm
% that hard codes the answer for $f$ and merely checks this answer.
% Instead, call a possible cost $C$ \emph{valid} if $C \in \costs(f)$ for some $f$ 
% defined over the fixed mesh $\MM$\footnote{If one is concerned
% that $C$ might be irrational, define valid if $2^C = 2^{\cost(f)}$.}. We prove
% a refined lower bound matching \Thm{main-alg}. 
% 
...
% \end{definition}
% 
% \begin{theorem} \label{thm:main-lb} Consider the set of functions $\bF_C$ (defined over a fixed mesh $\MM$) 
% that all have the same value $C$ as a valid cost.  Any height function decision tree that correctly computes
% the contour tree on all of $\bF_C$ has an average case (uniform over $\bF_C$) running time of $\Omega(C)$.
% \end{theorem}

In other words, it is not possible to improve on \Thm{main-alg} even when restricted to a subset
of inputs of fixed cost.

\paragraph{Beyond worst-case analysis:} Our result provides understanding of the complexity
of contour tree construction beyond the standard worst-case analysis. Our theorem and lower bound explain \emph{when}
computing contour trees is expensive, and provide insight into the hardness of particular instances. It is intriguing
that the cost (as defined earlier) roughly captures the hardness of an instance.
In that sense, it is related to the concept of instance optimal algorithms, as defined by Afshani \etal \cite{abc-ioga-09}.
...
\ignore{
classic output sensitive convex hull algorithms of Kirkpatrick and Seidel~\cite{KiSe86} and Chan~\cite{c-ooscha-96}.
...
Contour trees are a special case of Reeb graphs, a general topological representation for real-valued function
...
\begin{figure}[h]\centering
    \includegraphics[width=.35\linewidth,height=.15\linewidth]{figs/balanced3}
    \hspace{.3in}
    \includegraphics[width=.35\linewidth,height=.15\linewidth]{figs/trees}
    \caption{On top, a surface will a balanced contour tree, but whose join and split trees have long tails.  
    On bottom (from left to right), the contour, join and split trees.}
    \label{fig:sorting}
\end{figure}
...
which provides one of the first rigorous, Morse-theoretic defintions of contour trees. All our assumptions and definitions
...
%Abusing notation, $\MM$ refers to this encoding, so we do not refer to $f$.
...
Morse degree will have some $>1$ degree in $\reeb(\MM)$.
...
with exactly the same contour tree, but different orderings of (heights of) the critical point.
Turning it around, we cannot deduce the height ordering of critical points from the contour tree. Clearly,
sorting the critical points is computationally reduntant for constructing the contour tree.
...
on the left (or among the depressions in the right) to compute the contour tree. Yet some ordering
information is necessary: in the left, the little valleys are higher than the big central valley,
...
are the connected components of $\MM$ with height greater than $h$. For a terrain, the superlevel
...
Formally, let $\MM^+_v$ denote the simplicial complex obtained by taking only those
vertices higher than $v$.

\begin{definition} 
\label{def:int-criticalJoin}
The critical join tree $\jc(\MM)$ is built on the set $V'$ of all critical points. 
...
the contour tree from the join and split tree. So the bottleneck is computing these trees. And observe
in \Fig{sorting} that the split vertices form a long path in the join tree (and vice versa). Constructing
these trees forces a global sort of the splits, a redundant computation. Unfortunately (unlike \Fig{sorting}),
the heights of joins and splits may be interleaved in a complex manner. The final merging of~\cite{csa-cctad-00}
...
Our aim is break $\MM$ into smaller pieces, where this redundant computation can be saved.

%
%{\bf Punchline:} We give a linear time procedure that partitions $\MM$ into smaller pieces. We guarantee
%that the contour tree of each individual piece is basically the join (or split) tree. This saves
%some of the redundant computation. We then show to put it all together the get the full contour tree.
...
\begin{figure}[h]\centering
    ...
    \caption{On left, downward rain spilling only (each shade of gray represents a different color), producing a grid.  On right, flipping the direction of rain spilling.}
    \label{fig:order}
\end{figure}
...
(Our real definition is more complicated, and involves simplicial complices
that have many minima.) Analogously, we can define maximum dominant simplicial complices,
...
and imagine torrential rain at the maxima. The water flows down, wetting any point that has a non-ascending
...
are a set of contours, given the by ``water line". The wet part is clearly maximum dominant, since all
...
we are left with a set of disconnected dry parts, and we could recur this procedure on them.
...
maximum dominant simplicial complices, the final complexity would be forbiddingly large.
...
\begin{theorem} \label{thm:int-rain} There is an $O(N)$ time procedure that cuts $\MM$ into extremum dominant simplicial complices
$\MM_1, \MM_2, \ldots$. Furthermore, given the set of contour trees $\{\cC(\MM_i)\}$, $\cC(\MM)$ can be constructed in $O(N)$ time.
\end{theorem}

\paragraph{Extremum dominance simplifies contour trees:} We will focus on minimum dominant simplicial complices $\MM$. By \Thm{int-rain}, it suffices
...
Water poured at any critical point must flow to the boundary, and the entire wet portion are connected.
...
one may think that that it suffices to design faster algorithms for join trees. But this
cannot give the sort of optimality we hope. Again, consider \Fig{sorting}. Any algorithm
...
of building the join trees can be related to the cost of building $\cC(\MM)$.
...
paint the interior of facets.} Alternately, this process is restricted to the 1-skeleton of $\NN$.
...
dominant input, using water to divide the input $\NN$ raise the complexity too much.
...
Let us focus on an initial painting, where the colors have 1-1 correspondence with the maxima. Refer to \Fig{colors}.
...
(So ignore the yellow mound in \Fig{colors}.)
...
The merging of the colors is not explicitly as that would be too expensive; we maintain a union-find
...
\begin{figure}[h]\centering
    \includegraphics[width=.27\linewidth]{figs/mounds}%
    \hspace{.35in}
    \includegraphics[width=.27\linewidth]{figs/mounds2}%
    \hspace{.4in}
    \includegraphics[width=.22\linewidth]{figs/mounds3}
    \caption{On the left, three initial colors.  In the middle, yellow and blue merge into green.  On the right, green and red merge into purple.}
    \label{fig:colors}
\end{figure}
...
All the non-heap operations can be easily bounded as $O(t\alpha(t) + N)$ ($t\alpha(t)$ from the union-find data structure
for colors). It is not hard to argue that at all times, any heap always contains a subset of a leaf
to root path. Unfortunately, this subset is \emph{not} contiguous, as \Fig{colors} shows. \Sesh{explain}
...
We employ a variant
of \emph{heavy path decompositions}, first used for Sleator and Tarjan for analyzing link/cut trees~\cite{st-dsdt-83}.
The final analysis is basically charges expensive heap operations to long paths in the decomposition.
...
entropy argument, any height decision tree that correctly computes the contour tree on all instances
requires average case $\Omega(\cost(P(T)))$ time. We prove that our algorithm makes $\Theta(\cost(P(T)))$
height comparisons on all these instances. In general, any algorithm requires $\Omega(C)$ time
...
that our algorithm requires $\Theta(\cost(P(T)))$ on all these instances.
...
have already determined the contour trees of the simplicial complexes
...
In this section we prove \Thm{main-lb}, but for join trees instead of contour trees.  Observe that given a fixed $(f,\MM)$, 
our algorithm for computing $\jc(\MM)$ is deterministic and so the path decomposition $P=P(\jc(\MM))$ in \Thm{runtime} is fixed, 
allowing us to define the cost, $\cost(f)=\sum_{p\in P} |p|\log|p|$.

%In the following note that two distinct terrains can have the same graph structure defining their meshes, but different height values associated with their vertices.
...
%    \includegraphics[width=.095\linewidth]{figs/sideView}
    \caption{Left: angled view of a diamond / Right: a parent and child cone put together}
    \label{fig:diamond}
\end{figure}

\begin{lemma}
\label{lem:pathDecomp}
Let $\cost(f)=\sum_{p\in P} |p|\log|p|$, for some height function $f$ defined over a complex $\MM$.
Then there is a family of functions, $F$, all defined over a common triangulation in plane, 
such that the size of $F$ is $\Pi_{p_i\in P} (|p_i|-1)!$, and no two function in $F$ define the same join tree.
\end{lemma}
\begin{proof}
 We describe a simple triangulation, with heights specified for all vertices except for the saddles.  The main idea is that 
 ...
 First, we describe the basic building block of these terrains, which roughly speaking corresponds to a fixed path $p\in P$.  
 Specifically, we say a \emph{diamond} of size $m$ is the following terrain.  To start, construct a slightly tilted cycle 
 of length $m$.  Specifically have two antipodal points, at heights $1$ and $0$, which we will call the anchor and trap of the diamond, respectively.
 The remaining $m-2$ are evenly spread between on the cycle and heights decrease monotonically when going from the anchor to the trap.
 Next, create an apex vertex at some appropriately large height, and add an edge between it and each vertex in the cycle.  At this point 
 we have built an upside down cone with $m$ triangular faces (see \Fig{diamond}). 
 %To make the terrain fully triangulated, we add a triangular outer face around the cycle. 
 %Specifically, add a triangle with vertices at heights $0$, $1/20m$, and $1/10m$, such that in the projection to the plane 
 %this triangle contains the $m$ length cycle, and there is an arbitrary triangulation of the space between this triangle 
 %and the cycle.  For consistency, the inner cycle of a diamond will always have at length at least $3$, i.e. even if $m\leq 3$.
 %
 %Now above each face of the cone of the diamond, we will create a triangular ``platform'', which will act as the location to glue on the bounding triangle of other (shrunken) diamonds, 
 ...
 %$h$ is some unspecified height value.  This near horizontal platform is then connected to its respective face with (nearly) vertical edges down to the face. See \Fig{diamond}.
 
 Now we describe how to attach two diamonds.  In this process, we glue the base of a scaled down ``child'' diamond on to a triangular cone face of the large  ``parent'' diamond (see \Fig{diamond}).
 Specifically, the anchor of the child diamond is attached directly to a face of the parent diamond at some unspecified height $h$. The remained of the base of the child cone is then 
 extended down until it hits the face of the parent.
 
 At this point we can construct the entire terrain, by repeatedly gluing together diamonds.  
 Specifically, for each path $p_i\in P$ we create a diamond of size $|p_i|+1$.
 The two faces adjacent to the anchor are always empty, and the remaining faces are for gluing on other diamonds.
 (Note that diamonds have size $|p_i|+1$, since path $|p_i|-1$ faces represent the joins of $p_i$, the apex represents the leaf, 
 and we need two empty faces next to the anchor.)
 Now we glue together diamonds of different paths in the same way the paths are connected in the shrub $\pathTree$.  
 ...
 this implies that, regardless of the heights assigned to the platforms, the join tree has a path decomposition whose corresponding shrub is equivalent to $\pathTree$.  
 
 Now there is a valid instance of this described construction for any relative ordering of the heights of the saddles on a given diamond.  
 Let the set of height functions for these instances be denote $F$.
 In particular, there are $(|p|-1)!$ possible orderings of the heights of the saddles on the diamond for $p$, and hence $\Pi_{p_i\in P} (|p_i|-1)!$ possible 
 functions in this set.  Also, each one of these functions will result in a different join tree.  Specifically, a permutation of the heights on a given diamond 
...
\begin{lemma}
\label{lem:cost}
 For any $f\in F$, $\cost(f) = \sum_{p\in P} |p|\log|p|$
\end{lemma}
\begin{proof}
First observe that by construction, regardless of the choice of $f\in F$, 
there is some path decomposition $\overline{P}(T)$ of the join tree such that 
each path $p$ consists exactly of the set of critical points (except for its anchor) from a given diamond (i.e. $\overline{P}(T)$ is the same for any $f\in F$). 
We now show that regardless of how paint spilling is done, this will be the maximum 
path decomposition of $T$.

 So consider any order of paint spilling over the surface defined by $f$.
 Consider any join on a diamond.  This join is the anchor point of some connecting child diamond.  
 The claim is that the two colors of this join (according to leaf assignment, \Def{leafAssign}, used in \Lem{runTimeUpper})
 are the colors of the apexes of the child and parent diamond.  
 This easily follows since this join has two up-stars, each of which has exactly one edge.  
 ...
 So let $c$ be the color of some maxima, $m$.  This claim implies that in the join tree, the vertices for 
 which $c$ appears are exactly the vertices in path in $\overline{P}(T)$ corresponding to the diamond of $m$
 and the vertex to which the root of this path connects to.
 This in turn implies that $\overline{P}(T)$ is a maximum path decomposition and hence $\cost(f) = \sum_{p\in \overline{P}} |p|\log|p|$
 To see this, consider a vertex ..... (remark that ties can be broken consistently, and so the there is one maximum decomp)
 
%  All these joins have an edge which connects 
%  directly to the maxima of that diamond.  In particular, all these joins have 
%  an upstar which is entirely colored by the color of this diamond's maxima.  
%  Therefore, in the leaf assignment of the join tree, determined by this painting (see \Def{leafAssign})
%  all these join get assigned a common color.  
%  In particular, all vertices in each path in $\overline{P}$ get assigned a common color.  
 
%  Now consider any vertex $v$ in $T$, and let $p$ be the path that contains it.  
%  Consider the child of $v$ that is not in $p$, and let $S(v)$ be the set of all the colors in this subtree.
%  The claim is that $v$ is the last occurence (i.e. closest to the root) of any color in $S(v)$.
%  We prove this by induction on the distance of $p$ from the root of the shrub.
%  For the base case, let $p$ be the root of the shrub, and let $v\in p$.
%  Let $c$ be any color in $S(v)$.  Suppose there is some vertex $w$ in between $v$ and 
%  r_p which is colored $c$.  This clearly cannot happen since $w$ must have the color of the leaf of $p$, 
%  and its second color must come from S(w) which does not contain $c$.
%  Now for the induction step, let $p$ be some path, and suppose the statement holds for $p'$ 
%  closer to the root of the shrub.
%  
%  
%  This vertx gets two colors,  e from the path p that it is in 
%  and the from the set of all colors of the subtree of the child not in $p$.  Call this set of The claim is that regarless of which color 
 

 \Ben{I am stuck here.}
%  One can now prove by induction (on the distance of a path from the root of the shrub), 
%  that if all vertices in every path have a common color, then that color can only appear 
%  in that path and the vertex to which that path connects (which then trivial implies this 
%  is a maximum path decomposition).  
%  First observe that this trivially holds for the path containing the root of $T$.  
%  For the induction step, consider any path $p$, and let $p'$ be the path 
%  which the root of $r_p$ of the path $p$ connect to.
%  Specifically, let $v$ be the vertex of $p'$ at which $p$ connects.
%  
%  So consider any vertex $v$ in the path $p$ which contains the root of $T$.  
%  Now $v$ has two children, one child $c_1$ in $p$ and some other child $c_2$ in another path $p'$.
%  %In order to show this is a maximum path decomposition we must argue $H_{c_1}\geq H_{c_2}$.
%  The claim is that $H_{c_2}$ cannot contain any vertex of $p$ other than $v$.
%  Specifically, consider any vertex on $p$ between the root and $v$.  This vertex must 
%  get assigned two color, one from each of it subtree.  It is already assigned a color from 
%  the subtree of $c_1$ hence its second color cannot come from the subtree of $c_2$, 
%  since both $c_1$ and $c_2$ are in the same subtree of this vertex.
%  This proves the base case, and the induction step follows by the same argument.
%  
%  One can now prove by induction (on the distance of a path from the root of the shrub), 
%  that if all vertices in every path have a common color, then that color can only appear 
%  in that path and the vertex to which that path connects (which then trivial implies this 
...
The following is the analogous statement to \Thm{main-lb}, but for join trees.
...
\end{theorem}
\begin{proof}
 If $C$ is a valid cost then there some $f$ such that $\cost(f)=\sum_{p\in P(T)} |p|\log|p|$, where $P(T)$ is the maximum path decomposition of the join tree defined by f.
 ...
 Therefore, any height function decision tree for the set $\bF$ must distinguish between these possibilities, and therefore must 
...
Then there is a set of at least $\Pi_{p_i\in P} (|p_i|-1)!$ distinct terrains, all of which have the same defining mesh.
\end{lemma}
\begin{proof}
...
\end{figure}
 ...
 So for each $P_i$ construct a surface mesh as described in \Lem{pathDecomp}.  By construction, the diamonds corresponding to the paths in $P_i$ are connected 
...
%     \caption{On the left, a surface will a balanced contour tree, but whose join and split trees have long tails.  
%     On the right (in clockwise order from the top) the contour, split and join trees.}
%     \label{fig:sorting}
% \end{figure}
