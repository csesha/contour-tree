...
\InSoCGVer{See Appendix~\ref{sec:techmarks} for further technical remarks on the above definitions.}
...
and $v$ is adjacent to a vertex in this component (in $\MM$). 
\end{definition}
...
where the input is just a triangulated terrain, the main ideas of the parts of the algorithm 
can be explained clearly. Therefore, in Appendix~\ref{sec:approach} we provide a high level view of the entire result.
...
\InSoCGVer{The following theorems are intuitively obvious, and are proven in Appendix~\ref{sec:proofOfSurgery}.}
...
since otherwise faces may be cut a super constant number of times (see \Fig{order}\InSoCGVer{ in Appendix~\ref{sec:approach}}).
In other words, when
...
Precise definitions and proofs are given Appendix~\ref{sec:jstedm}.
}
...
and $v$ \Ben{is adjacent to a vertex} in this component (in $\MM$). 
\end{definition}

\InSoCGVer{Each non-dominant minimum, $m_i$, connects to the contour tree at a corresponding split $s_i$. We have the following (see Appendix~\ref{sec:jstedm} for more details).}
...
\label{sec:runTime}
\Ben{This section needs to be reduced dramatically in size.}
...
\InSoCGVer{
\Ben{Summarize Appendix~\ref{sec:pathDecomp} on path decomposition upped bound here.}
}
...
\InSoCGVer{
\Ben{summarize Appendix \ref{sec:lb}, the lower bound section, result here.}
}
\InNotSoCGVer{
