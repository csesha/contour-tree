...
These points are the ``events" where the contour topology changes and form the vertices of the contour tree. (This is formally incorrect,
but gives the right intuition.)
...
\begin{asparaitem}
	 \item For every maximal connected component $Y$
     of $\MM^+_v$, the set of all edges connecting $v$ to $Y$
     is called an \emph{up-star}.
...
The primary challenge to use the data structures to rapidly find
ripe vertices.
...
The pieces were create are technically not simplicial complices,
in that each piece may have facets that not simplices. We may
create facets of larger size than the dimension. These can
easily be completeed into simplicial complices, though we do not
explicitly need this. For ease of exposition, we do not explicitly
