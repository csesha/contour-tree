...
the topology of the level set changes. The contour tree is a concise representation of all these events.\footnote{%
...
\Ben{Let $f:\MM \to \RR$ be the linear interpolant over distinct valued vertices, where the contour tree $T$
has maximum degree $3$}.\footnote{The degree $\leq 3$ assumption helps simplify the proof and presentation, and is often made, as in \cite{kobps-ctsssit-97}.}\Ben{See footnote} 
There is a deterministic algorithm to compute $T$ whose running time is $O(\sum_{p \in P(T)} |p|\log |p| + t\alpha(t) + N)$,
where $P(T)$ is a specific path decomposition (constructed implicitly by the algorithm)\Ben{More than one reviewer has complained about the running 
time being stated as a function of our algorithm.  So Rephrase, and maybe discussion about connection to sorting.}.
...
Consider a contour tree that is constant balanced, meaning that the ratio of sizes of sibling subtrees
is $\Theta(1)$. 
A calculation yields that for any decomposition of such a contour tree, 
...
so $t = \Omega(n)$. The running time of the algorithm of \Thm{main-alg}
...
Abusing notation, for now we will just call this the join tree. (In \Sec{extreme} we carefully handle the distinction 
...
\Ben{New section, read carefully:}
...
\myparagraph{Outline of the algorithm and the following sections.} 
The high level idea is to partition $\MM$ into nicely behaved pieces, on which we know how to compute the contour tree optimally. 
...
Finally, \Sec{runTimeSummary} summarizes all sections relating to the running time, specifically \Sec{runTime} which upper bound the 
...
Precise definitions and proofs are given \Sec{jstedm}.
}
...
In the following a rain consistent path decomposition is a decomposition which is the union of paths from 
any decomposition on each subcomplex output by $\rain$ (see Definition~\ref{def:path2} for a formal definition).
...
% whose size is that described in the lemma statement. 
 ...
 Specifically, the anchor of the child diamond is attached directly to a face of the parent diamond at some height $h$. The remainder of the base of the child cone is then 
...
% so the second color is that of the maxima of the child diamond.
 ...
 any ancestor of $v$. This subtree is exactly the join tree of the child diamond attached at $v$.
