...
\label{lem:pathBounds}
 ...
 \begin{compactenum}
  ...
  \item for any light path $p\in P(T)$, $\sum_{v\in P} |H_v| = O(|p| \log |H_{r_p}|)$.
 \end{compactenum}
\end{lemma}
\begin{proof}
...
\label{lem:adjacent}
 ...
 Let $H_i$ be the heap at $r_{q_i}$ and let $H_0$ be the heap at $r_p$.  Then $\sum_{i=1}^k |H_i| \leq |H_0|+|p|$.
...
\begin{proof}
 ...
 First observe that for all $i$, $r_{q_i}$ lies above $r_p$ on some root to leaf path. 
 Let $H_i'$ be the subset of $H_i$ that lies below $r_p$.  Observe that $H_i'\subseteq H_0$, as $L(T_{r_{q_i}})\subseteq L(T_{r_p})$.
  
 Now for any $i\neq j$, $H_i \cap H_j = \emptyset$, as $r_{q_i}$ and $r_{q_j}$ are not on the same root to leaf path.
 ...
 Moreover, for any $i\neq j$, $ H_i' \cap H_j' = \emptyset$, and so since $H_i'\subseteq H_0$ we have $\sum_{i=1}^k |H_i'|\leq |H_0|$.
 Putting these together gives
 \[
  \sum_{i=1}^k |H_i| = \sum_{i=1}^k |H_i'| + \sum_{i=1}^k |H_i\setminus H_i'| \leq |H_0|+|p|.
 \]
\end{proof}
...
\begin{proof}
 ...
 Now by \Lem{adjacent}, $\sum_{i=1}^k |H_i| \leq |H_0|+|p| = O(|p|)$, as $p$ is a heavy path. 
...
For a given coloring $\chi$ of a merge tree $T$, we now describe a particular path decomposition such that 
$f_{alg}(\chi(T)) = \Theta(f_{path}(P(T)))$.  By \Lem{algCost} and \Lem{pathDecomp} this will imply the running time of our algorithm 
is optimal with respect to the shape of the merge tree.
...
%We define two cost functions over validly colored trees.  To do so, we first define two quantities for each vertex $v\in T$.  
%First let $P_v$ be the longest path from $v$ to a descendant leaf of $v$.  Second, let $H_v$ be the subset of 
%vertices along the path from the root to $v$ such that one of the two colors  is in $\chi(L(T_v))$.
