...
Consider any valid path decomposition $P$ of a valid join tree (actually, for any rooted binary tree,
there exist a terrain for this is the contour tree).
When we say ``compute the join tree", we require the join tree to
be labeled with the corresponding vertices of the terrain.


%
%For given $\MM$, consider running the algorithm of \Sec{also} to compute $\jc(\MM)$.
%Let $P(\jc(\MM))$ be the maximum path decomposition of the join tree according to the initial coloring
%of $\init(\MM)$. Define $\cost_{\jc}(f) = \sum_{p\in P(\jc(\MM))} |p|\log|p|$
...
Fix a valid path decomposition $P$. There is a family of terrains, $\bF_P$, all with the same triangulation, 
...
% whose size is that described in the lemma statement. 
 ...
 Next, create an apex vertex at some appropriately large height, and add an edge each vertex in the cycle. 
...
\begin{lemma}
\label{lem:cost}
For all $\MM \in \bF_P$, the total number of heap operations performed by $\build(\MM)$ is $\Theta(\sum_{p\in P} |p|\log|p|)$.
\end{lemma}
\begin{proof}
...
We show that regardless of how paint spilling is done, the number of heap operations is bounded as above.
 ...
 Consider an arbitrary order of the initial paint spilling over the surface defined by $f$.
 ...
 Hence, two colors touching this join (according to \Def{initialColoring})
