...
\begin{claim} \label{clm:above} Let $Q$ be a set of incomparable nodes in the shrub $P(T)$ (which are paths in $T$).
Suppose all paths in $A(Q)$ are heavy.
Then $\sum_{q \in Q} |H_q| \log |H_q| = O(\sum_{p \in A(Q)} |p|\log |p|)$.
\end{claim}

\begin{proof} Pick $q \in Q$ and some $v \in H_q$. As we traverse up the tree passing
...
\begin{lemma} \label{lem:light} $\sum_{p \in L} \sum_{v \in p} \log |H_v| = O(\sum_{p \in L'} |H_p| \log |H_p|)$.
\end{lemma}
...
We have $\sum_{v \in T} \log |H_v| = \sum_{p \in L} \sum_{v \in p} \log |H_v| + \sum_{p \notin L} \sum_{v \in p} \log |H_v|$.
For the first term, we apply \Lem{light} to bound by $O(\sum_{p \in L'} |H_p| \log |H_p|)$.
Now, we apply \Clm{above} to the set $L'$, noting that it satifies the claim conditions and that $A(Q) \cap L = \emptyset$.
Hence, the first term is $O(\sum_{p \notin L} |p|\log |p|)$.
The latter is also bounded by the same expression, by \Lem{pathBounds}. By definition,
$\sum_{p \notin L} |p|\log |p| \leq \cost(\pmax(T))$.
\end{proof}
