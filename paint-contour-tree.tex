...
\begin{definition}
\label{def:path} 
For a contour tree $T$, a \emph{path decomposition}, $P(T)$, is a partition of $T$ into a set of vertex disjoint leaf paths.  
\end{definition}
...
specific path decomposition $P(T)$%
\FullVer{
\footnote{The specific decomposition used is defined in \Sec{pathDecomp}, 
...
\SubmitVer{
\footnote{The specific decomposition used is defined in the full version of the paper.  
The important thing is that our algorithm is deterministic, and so for a fixed $\MM$ and $f$ it is well defined.}.}
