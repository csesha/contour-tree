...
Note that if one intersects $\MM$ with given ball $B$, then a single contour in $\MM$ might be split into more than one contour in the intersection. 
In particular, two $(f(x)+\delta)$-contours in $f|_{B_\eps(x)}$, given by \Def{deg}, might actually be the same contour in $f$. 
