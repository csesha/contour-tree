...
THERE IS A PROBLEM in the following lemma.
...
\begin{definition}
 ...
 Call this set of reachable paths the \emph{light path set} of $p$.  
 Consider the natural tree over the paths in the light path set of $p$, where each vertex in the tree is a light path 
 $p$ is the root.  We call this tree the \emph{light path tree} of $p$
\end{definition}

Let $\mathcal{L}$ be a light path tree of some light path $p$ in a maximum path decomposition $\mathcal{P}(T)$.
For a path $v\in \mathcal{L}$, let $H_v$ denote the heap at the root of $v$, and let $H_{v_f}$ denote the heap 
at the leaf of $v$.  Let $S_l(v)$ (resp. $S_h(v)$) be the subset of light (resp. heavy) paths adjacent to $v$ in $T$
(i.e. $S_l(v)$ are the children of $v$ in $\mathcal{L}$).  Then define the cost of $v$ as 
$c(v) = H_{v_f}+\sum_{i\in S_h(v)} H_i$.

We now define a weighted version of $\mathcal{L}$ based on these cost.  First we augment $\mathcal{L}$ by attaching one 
leaf to each vertex in $\mathcal{L}$.  Now for each newly created leaf (which are the only leaves now) assign the cost 
$c(v)$ defined above, where $v$ is that leaf's corresponding parent.  Now for any vertex $u$ in this tree, define 
its weight to be the sum of the weights of its children (where at the leaves the weights are just the costs).  
To simplify notation, we will also refer to this new weighted tree as $\mathcal{L}$.

As the weight $w(v)$ includes the sizes of the heaps of all leaves in the subtree rooted at $v$, we have the following.

\begin{claim}
For any $v\in \mathcal{L}$, $w(v)\geq |H_v|$
\end{claim}

Let $W= \sum_{v\in \mathcal{L}} w(v)$.  Suppose one could prove $W=O(H_r)$, where $r$ is the root of $\mathcal{L}$.  
Then by the above claim, 
$\sum_{v\in \mathcal{L}} H_v = O(H_r)$, and so by convexity and \Cor{heavyLight}, we could argue the entire cost 
of all light paths in the light path tree can be charge to a heavy path.  So proving $W=O(H_r)$ is our new goal.
