...
unsupported paths, though doing so is much trickier.  The following is a key property of unsupported paths which 
...
\begin{corollary}\label{cor:geometric}
 Let $\cU$ denote a connected component of $\cF$. Let $p$ and $d$ be any two nodes in $\cU$ such that $d$ is a descendant of $p$.
 Then for any $v \in d$, $|H_v| = O(|H_p|)$.
\end{corollary}
...
\begin{definition}\label{def:buff}
 Let $\cU'$ be a pruning of a shrub of $\cU$ of $\cF$. Then a rooted subtree $\buff$ of $\cU'$ is called a \emph{buffered sub-shrub} if the following conditions hold:
  \begin{enumerate}
...
The following will allow buffered sub-shrubs to be charged to their tall support chains.
...
The goal of this section is to prove the following lemma, which ultimately allows us to pay for pruned shrubs.
...
Observe that part (i) of \Lem{cu-root} is immediate, as it is a special case of \Cor{geometric}. Proving part (ii) requires much more work.
...
\begin{proof}[Proof of \Lem{cu-root}]
...
\textcolor{red}{\huge Ben will update this subsection to be consistent with new changes}
