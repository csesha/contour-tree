...
\begin{corollary}\label{cor:geometric}
 ...
 Then for any $w \in d$ and $v \in \lambda(p)$, $|H_w| = O(|H_v|)$.
\end{corollary}

\begin{proof} By the previous lemma, $|H_w| = O(|H_p|)$. Note that there are at most
$|H_p|/100$ nodes in $\lambda(p)$, since these are all in the tall support chain of $v$
...
subtree of $p$ in $\cU'$. We simply check if $\sum_{q \in \cU'_p} |\loss(p)| \geq \sum_{q \in \cU'_p} |q|/100$. If so, we prune off $\cU'_p$. (Note that $\cU'$ is redefined
to be the remainder.)
...
to a buffered sub-shrub to be charged to tall support chains.
...
We reserve topologically sort (descendants before ancestors) the nodes in $\buff$.
...
For a leaf $\ell$ in $\pruned$, set $\alpha_\ell = |H_\ell|$. Consider internal node $p$
with children $q_1, q_2, \ldots, q_k$. The heap $H(p)$ is formed by the union
of the $H(q_i)$ heaps, after deleting certain vertices from them, and potentially
...
Thus, we have $|H_p| = \sum_{i \leq k} |H_q| + \alpha_p$
$- \textrm{\# vertices deleted from $H_q$s}$.
...
\leq \sum_{q \in \pruned_p} |q|/100$. This completes the proof.
\end{proof}
...
equation. Ae use the assumption that $A_{a_i} \geq (5/4) A_{a_{i-1}}$ and hence $A_{a_i} \geq (5/4)^i A_{a_0} \geq (5/4)^i$, 
as $a_0$ is an unsupported short path and hence $A_{a_0}\geq 1$. 
$$ ({\alpha_q}/99)\sum_{a_i} 1/\sqrt{A_{a_i}} \leq ({\alpha_q}/99)\sum_{a_i} (5/4)^{i/2} \leq \alpha_q/20 $$
The total charge is $\sum_{q \in \pruned_p} \alpha_p/20 = A_p/20$. 
\end{proof}

\begin{claim} \label{clm:exp-A} For any $q, q' \in \pruned_p$ where $q$
is a parent of $q'$, $A_q \geq (4/3)A_{q'}$.
\end{claim}

\begin{proof} We prove this by induction over the depth of $q$.
When $q$ is a leaf, this is vacuously tree. Consider some internal node $q$,
...
By \Clm{weights}, $|H_{q'_i}| \leq 17A_{q'_i}/20$ and $A_q \geq |H_q|$.
...
we get that $\sum_{q' \in \pruned_{q_i}} |q| \leq A_{q_i}/20$.
Summing this over the children, $\sum_{p \in \pruned, p \neq r} \leq A_r/20$.
By \Clm{weights}, $A_r \leq 17|H_r|/20$. Furthermore $|r| \leq |H_r$.
