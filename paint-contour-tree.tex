...
\begin{definition}
 ...
 one from the left and one from the right subtree of $v$.
\end{definition}
...
Fix $\chi(T)$, which fixes all the heaps $H_v$. We choose a special path decomposition, which can be related
to the running time of the algorithm. This is defined best as a subset of edges in $T$ such that each internal
vertex has degree exactly $1$. This naturally gives as path decomposition.
...
\begin{theorem} \label{thm:runtime} $\sum_{v \in T} \log |H_v| = O(\sum_{p\in P(T)} |p| \log |p|)$.
\end{theorem}
...
Consider $v \in \bigcup_{i} H_i$ that lies below
$b(p)$ in $T$. Note that $v \in H_p$ and has only one color in $\bigcup_{i} L(T_(b(q_i))) \cup \ell(p)$.
Since the latter sets are all disjoint, any $u \in H_i \cap H_j$ must lie in $p$.
Any $u \in p$ can be in at most two of the $H_i$'s. Hence, $\sum_i |H_i| \leq |H_p| + 2|p|$.
\ignore{
...
There are some light paths that we can also ``pay" for. 

\begin{definition} \label{def:support} A light path $p$ is \emph{supported} if
at least $|H_p|/100$ vertices $v$ in $H_p$ lie in heavy ancestors.
%
%
% there exists
...
Most of work goes into proving the following technical lemma.
...
which mostly imvolves routing calculations. Then we prove \Lem{cu-root}, where most
of the interesting work happens.
...
The following claim shows that in turn is at most the last term.
...
%
%
As we traverse the heavy ancestors, there
...
Pick another $\widetilde{p} \in \cL'$. If $p$ and $\widetilde{p}$ are ancestor-descendant,
then their corresponding supporting ancestor chains are disjoint. Any $v \in H'_p$
can only occur in one other $H'_{\hat{p}}$ (for $\hat{p} \in \cL'$), because $v$ has exactly two colors.
...
We have $\sum_{p \in \cL \setminus \cL'} \leq \sum_{\cU} \sum_{p \in \cU} \sum_{v \in p} \log |H_v|$.
...
\begin{lemma}
\label{lem:geometric}
 Let $p$ be the parent of $q$ in the shrub $\cU$. Then $|H_p|\geq \frac{3}{2}|H_q|$. 
\end{lemma}
\begin{proof} By construction, there is a heavy path (denoted $h(q)$) that is a child of $p$ in $\pmax(T)$,
and an ancestor of $q$. The chain of ancestors from $q$ to $h(q)$ consists only of heavy paths.
These paths contain at most $|H_p|/100$ vertices of $H_p$, so $|H_{h(q)}| \geq 99|H_p|/100$.
...
implies $|H_v| \geq |H_w|$. Since $|H_u| \geq |H_v| + |H_w| - 1$,
$|H_u| \geq 2|H_w| - 1$. Let $b$ be the base of $p$.
We have $|H_p| = |H_b| \geq |H_u| - |p| \geq 2|H_w| - |p| - 2$. Since $p$ is a light path, $|p| < \sqrt{|H_p|}/100$.
Applying this bound, we get $|H_p| \geq (2-\delta)|H_w|$ (for small constant $\delta > 0$).
Since $w$ is the base of $h(q)$, $H_w = H_{h(q)}$. We apply the bound $|H_{h(q)}| > 99|H_q|/100$,
...
which has children $q_1, q_2, \ldots, q_k$ in $\pmax(T)$.
Furthermore, as a leaf path in $p$, some leaf
$\ell$ of $T$ in one of the ends of $p$. The heap $H_p$ is the
...
$q_1, q_2, \ldots, q_k$ in $\cU$. Then $R_p = |H_p| - \sum_i |H_{q_i}|$. 
By definition, $|H_p| = \sum_{q \in \cU_p} R_p$. 
Note that $R_p$ can be negative. 
Now, define $R^+_p = \max(R_p,0)$, and set $W_p = \sum_{q \in \cU_p} R^+_p$.
Observe that $W_p \geq |H_p|$. We also get an approximate converse.


\begin{claim} \label{clm:loss} $|H_p| \geq W_p - 2\sum_{q \in \cU_p} |q| - 2\sum_{q \in \cU_p} \sum_{h \in \cH} |h \cap R_q|$
\end{claim}

\begin{proof} Consider $q \in \cU_p$, and traverse $\pmax(T)$ up to $p$. Suppose we encounter
...
We stress that the all the previous work is mostly setup to get to this claim.

\begin{claim} \label{clm:charge} Consider $p \in \cU$ such that for any $q,q' \in \cU_p$
such that $q$ is a parent of $q'$, $W_q \geq (4/3)W_{q'}$. Then $\sum_{q \in \cU_p} |q| \leq W_p/20$.
\end{claim}

\begin{proof} By the definition of light paths, $|q| < \sqrt{|H_q|}/100 \leq \sqrt{|W_q|}/100$.
We prove that $\sum_{q \in \cU_p} \sqrt{|W_q|}/100 \leq W_p/20$
by a charge redistribution scheme. Assume that each $q \in \cU_p$
starts with $\sqrt{W_p}/100$ units of charge. We redistribute this charge over all
...
give $(\sqrt{W_q}/100)\cdot(R^+_{q'}/W_q)$ units of charge to each $q' \in \cU_q$.
...
equation. We use the assumption that $W_{a_i} \geq (4/3) W_{a_{i-1}}$ and hence $W_{a_i} \geq (4/3)^i$.
$$ ({R^+_q}/100)\sum_{a_i} 1/\sqrt{W_{a_i}} \leq ({R^+_q}/100)\sum_{a_i} (3/4)^{i/2} \leq R^+_q/20 $$
...
Suppose $W_q \geq (4/3)W_{q'}$ (for $q$ parent of $q'$). By \Clm{charge}, $\sum_{p \in \cU} |p| \leq W_{r}/20$.
By \Clm{loss}, $|H_r| \geq W_r - 2\sum_{p \in \cU} |p| - 2\sum_{h \in \cH} |h \cap H_p|$.



We first need to prove the claim assumption,
namely that of $W_q \geq (4/3)W_{q'}$ (for $q$ parent of $q'$). We do know from \Lem{geometric}
that $|H_q| \geq (3/2)|H_{q'}|$. We will apply \Clm{loss} and \Clm{charge} to prove this.



Pick the node furthest from $r(\cU)$
...
\begin{claim} \label{clm:loss} Consider $p \in \cL \setminus \cL'$ that is in $\cU$. Let $Q$
