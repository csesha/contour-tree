...
But if $P(T)$ is a rooted, balanced, binary tree, then a calculation yields $\sum_{p \in P(T)} |p|\log |p| = O(t)$.
...
\begin{figure}[h]\centering
    \includegraphics[width=.18\linewidth,height=.2\linewidth]{figs/orderGray1}%
    \hspace{.8in}
    \includegraphics[width=.18\linewidth,height=.2\linewidth]{figs/orderGray2}
    \caption{On left, downward rain only (each shade of gray represents a different iteration), producing a grid.  On right, flipping the direction of rain.}
    \label{fig:order}
\end{figure}
...
%We say two shrubs $S_1$ and $S_2$ (of join trees of potentially different surfaces) are equivalent if there is an isomorphism, $f$,
...
Let $T$ be a join tree, and let $P = P(T)$ any path decomposition of $T$.  %, and $\pathTree$ the corresponding shrub.  
