...
\newcommand{\CiteFullVer}{\cite{rs-mvrms-14}}%
...
\Copyright{Benjamin Raichel and Seshadhri Comandur}%mandatory, please use full first names. LIPIcs license is "CC-BY";  http://creativecommons.org/licenses/by/3.0/
...
We assume the input is $f:\MM \to \RR$ a linear interpolant over distinct valued vertices, where the contour tree
$T$ has maximum degree $3$. The degree assumption simplifies the proof and presentation, and is commonly made~\cite{kobps-ctsssit-97}.
...
trees joined at their roots, and we do not need to entire sorted order of critical points
to construct the contour tree. Is it possible to beat the $t\log t$ bound for these examples?
...
can be computed in $O(\sum_{v \in C(T)} \ell_v + t\alpha(t) + N)$ time. Moreover, this algorithm only
...
\subsection{A refined bound with optimality properties}

\Thm{main-corr} is a direct corollary of a more cumbersome theorem.
...
There is a deterministic algorithm to compute $T$ whose running time is $O(\sum_{p \in P(T)} |p|\log |p| + t\alpha(t) + N)$,
...
Note that \Thm{main-corr} is a direct corollary of this statement. This is strictly stronger,
since for any balanced contour tree, the run time bound is $O(t\alpha(t) + N)$, and $O(t)$
...
comparisons in the worst-case over this set.

\begin{theorem} 
\label{thm:main-lb}
There exists some absolute constant $\alpha$ such that the following holds.
For sufficiently large $t$ and $C$: consider the set $\bF_C$ of terrains with $t$ critical points such that
the number of comparisons made by the algorithm of \Thm{main-alg} on these terrains is in $[C,\alpha C]$.
...
This equivalence relation gives a set \emph{contour classes}. 
...
%
\Ben{Similar to \cite{kobps-ctsssit-97} (among others), multi-saddles are treated as a set of ordinary saddles, which can be realized 
via vertex unfolding (which can increase surface complexity if multi-saddle degrees are allowed to be super-constant).
Therefore, to simplify the presentation, for the remainder of the paper up and down-degrees are at most $2$, and the total degree is at most $3$.}
%
...
\InSoCGVer{See \Sec{techmarks} for further technical remarks on the above definitions.}
...
Analogously, we can define maximum dominant complexes, and both are collectively called extremum dominant.
...
the wet part, and recurse of the dry part. Each additional cut increases the complexity of the surface,
...
Using ideas from~\cite{csa-cctad-00}, we prove that the contour tree of an extremum dominant complex is (basically) just the \emph{join tree}.
...
Consider sweeping down via the hyperplane $x_{d+1} = h$ and taking the connected components of the portion of $\MM$ above height $h$. For a terrain, these superlevel
...
Denote the input as a terrain $\NN$.
...
paint to reach it.  We begin with an initial painting, since all maximum
colors are distinct.
...
So this indicates that $v$ joins the blue maximum and red maximum in $\jc(\MM)$, and is a critical point.

To process $v$, we ``merge" the colors red and blue into a new color, purple. 
In terms of $\jc(\MM)$, this
...
Abusing notation, for now we will just call this the join tree. (In \Sec{extreme} \InSoCGVer{and \Sec{jstedm}} we carefully handle the distinction 
...
\myparagraph{The running time analysis:} Relating the running time to a path decomposition is the most technical part of the paper. 
...
\InSoCGVer{The following theorems are intuitively obvious, and are proven in \Sec{proofOfSurgery}.}
...
has non-trivial intersection with $\wet(x,\MM)$ \Ben{(i.e.\ the intersection is neither empty nor the entire neighborhood)}.
\end{definition}
...
\Ben{Read remainder of this section carefully, it has been cut up, and parts rewritten to fit.}


\InNotSoCGVer{
The main theorem of this section asserts that contour trees of minimum dominant manifolds have a simple description.
...
is why we allowed for non-dominant minima and maxima in the definition of extremum dominant manifolds.  
The punchline is that, ignoring these stumps, the contour tree of an extremum dominant manifold is equivalent to its \emph{join} (or \emph{split}) tree, defined in~\cite{csa-cctad-00}.
Hence the critical join tree below is just the join tree without these stumps.  
Precise definitions and proofs are given in \Sec{jstedm}.
}
...
\begin{definition} \label{join} The \emph{join tree} $\cJ(\MM)$ of $\MM$ is built on vertex set $\cV(\MM)$.
...
and $v$ \Ben{is adjacent (in $\MM$) to a vertex} in this component. 
\end{definition}

\InSoCGVer{Each non-dominant minimum, $m_i$, connects to the contour tree at a corresponding split $s_i$. We have the following (see \Sec{jstedm} for more details).}
...
\InNotSoCGVer{\algorithmRunTimeSection}
%%%%%%%%%%%%%%%%%%%%
\InSoCGVer{
\subsection{Upper and Lower bounds for Running Time}\label{sec:runTimeSummary}
\Ben{This section is new, and only appears in the socg version}
The full running time analysis can be found in \Sec{runTime}, \Sec{pathDecomp}, and \Sec{lb}, or in the full version \CiteFullVer.  
The main results of these sections are summarized here.
%
In \Sec{runTime}, we give a tight bound on the running time of $\build(\MM)$, as a function of the choices made by the subroutine $\init(\MM)$.
To state this bound we require the following technical definitions (which are repeated in \Sec{runTime}).  
...
The subroutine $\init(\MM)$ naturally defines a leaf assignment to $\jc(\MM)$ (which 
is a rooted binary tree) according to the priority queue for each up-star we put a given vertex in.  
Call this the \emph{initial coloring} of the vertices in $\jc(\MM)$, and denote it by $\chi$.  

For a vertex $v$ in $\jc(\MM)$, let $L(v)$ denote the set of leaves of the subtree rooted at $v$, and let $A(v)$ denote the set of 
ancestors of $v$, i.e.\ the vertices on the $v$ to root path. For a vertex $v\in \jc(\MM)$, and an initial coloring $\chi$, 
...
The algorithm $\build(\MM)$ processes vertices in $\jc(\MM)$ one at a time.  The main processing cost  
comes from priority queue operations.  The heap as defined above characterizes the  
priority queue size at the time a vertex gets processed and hence we get the following.

\begin{lemma}
Let $\MM$ be a simplicial complex with $t$ critical points.  
The running time of $\build(\MM)$ is $O(N+t\alpha(t) + \sum_{v \in \jc(\MM)} \log |H_v|)$, where $H_v$ is defined by an initial coloring.
%For every vertex in $\jc(\MM)$, let $H_v$ be defined by the initial coloring of $\MM$.
%The running time of $\build(\MM)$ is $O(N+t\alpha(t) + \sum_{v \in \jc(\MM)} \log |H_v|)$.
\end{lemma}

While the above gives a full running time characterization, ideally the bound should be expressed as   
a function of some structural property of $\jc(\MM)$.
This is the focus of \Sec{pathDecomp}, where it is shown that for any binary tree $T$ 
the $H_v$ sets (defined by any leaf assignment) naturally induces a path decomposition of $T$ (see \Def{path}), 
denoted $\pmax(T)$ (see \Sec{pathDecomp} for details).   

% Specifically, the main result of \Sec{pathDecomp} is the following theorem, which together with the above 
% bound in terms of heap size immediately implies our main result, Theorem~\ref{thm:main-alg} .

\begin{theorem} $\sum_{v \in T} \log |H_v| = O(\sum_{p\in \pmax(T)} |p| \log |p|)$.
\end{theorem}

The above is a purely combinatorial statement on binary trees, and may find applications in other charging arguments.  
However, for the specific when $T=\jc(\MM)$, plugging this theorem into the above lemma, and applying this lemma 
to each subcomplex piece output by $\rain$, yields our main result, Theorem~\ref{thm:main-alg}.

The path decomposition $\pmax(T)$ is a function of the choices made by the linear time $\init(\MM)$.  
One cannot control which path decomposition $\init$ determines, as sorting can be reduced to finding the optimal path decomposition.  
Specifically, in \Sec{lb} we exploit this connection to prove the following matching lower bound for our approach.
In the following a rain consistent path decomposition is any decomposition which is the union of paths from 
decompositions on each subcomplex output by $\rain$ (see Definition~\ref{def:path2} for a formal definition).
...
% tree computing the join tree (on $\bF_P$) requires $\Omega(\sum_{p \in P} |p|\log |p|)$ time.
...
% 
% Note that in the above $T$ is assumed to be a rooted tree, and so the contour tree is equivalent to the join tree.   
% In general our input $\MM$ will not have a contour tree with this structure, which is why we required the cutting procedure of \Sec{rain}.  
% The above result can be extended to such general contour trees, however, here we restrict to rain consistent path decomposition, 
% i.e.\ decompositions which are the union of paths decompositions from each piece (see \ref{def:path2} for a formal definition)
...
% Let a path decomposition $P$ be called \emph{valid} if it is actually output by the algorithm on some input.
% \begin{theorem} 
% \label{thm:main-lb}
...
% \end{theorem}
% 
% In other words, for any path decomposition, $P$, that our algorithm chooses, we can design a hard family where any 
% algorithm requires $\sum_{p \in P} |p|\log |p|$ time in the worst case, and our algorithm matches this bound. 
}
%%%%%%%%%%%%%
...
Let $f:\MM \to \RR$ be the linear interpolant over distinct valued vertices, where the join tree $\jc(\MM)$ has maximum degree $3$.
...
\section{Join and Split Trees of Extremum Dominant Manifolds}
\label{sec:jstedm}
In this section we prove Theorem~\ref{thm:contour-tree} (of \Sec{extreme}), which states the equivalence 
of the contour tree on extremum dominant manifolds and the critical join tree of Definition~\ref{def:criticalJoin}.
This proof requires some definitions and notation, specially
the notions of \emph{join} and \emph{split} trees, as given by~\cite{csa-cctad-00}.
Conventionally, all edges are directed from higher to lower function value. 

\joinTreeSplitTreeMerge

% We now prove Theorem~\ref{thm:contour-tree} (of Section~\ref{sec:extreme}), which states the equivalence 
% of the contour tree on extremum dominant manifolds and the critical join tree of Definition~\ref{def:criticalJoin}.
