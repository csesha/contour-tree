...
%
\begin{definition} \label{def:paint} A  \emph{painting} of $\MM$ is a map $\chi:V \cup E \mapsto [|S|]$
with the following properties. (Remember that $G(\MM) = (V,E)$.)
\begin{asparaitem}
	\item The restriction $c:S \mapsto [|S|]$ is a bijection.
	\item Consider an edge $e$. There exists a descending path from some maximum $x$ to $e$
	consisting of edges in $E$, such that all edges and vertices along this path have the same color. (The maximum $x$
	is well-defined to be the unique maximum with the same color as $e$.)
\end{asparaitem}
\end{definition}
%
Note that paintings are not unique. It is straightforward to construct a painting of $\MM$ in $O(n)$ time, using a descending BFS from
each maximum. 
%
\begin{definition} \label{def:color-set} Fix a painting $\chi$, and a color $c$.
A critical point $x$ is \emph{$c$-touched} if $x$ is incident to a $c$-colored
increasing edge. The set of $c$-touched critical points is denoted $\touch(c)$.
\end{definition}
%
We will store each set $\touch(c)$ in a binomial heap. Abusing notation, $\touch(c)$ refers
