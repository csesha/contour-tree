...
We use key theorems from~\cite{Carr} (Theorem 6.6) that maps paths in $\cC(\MM)$ and $\MM$.
...
An \emph{initial} painting has the additional restriction that the restriction $\chi:X \mapsto [|X|]$ is a bijection.
\end{definition}
%
...
of edges that all connected to a fixed component of $\MM^+_v$. (Each $v$
...
while we only have access to the original painting. By union-find bounds, the only adds an $\alpha(|\MM|)$
...
\begin{asparaitem}
	...
	\item After $v$ is processed, all edges incident to $v$ has the same color.
\end{asparaitem}
...
There is obviously an edge of color $\rep(c)$ touching $h$ connected to a component $\SS$ of $\MM^+_v$. 
From the initialization, $h \in T(\rep(c'))$, for some $c'$ touching $h$ (where $c'$ is the color
of some edge connecting $h$ to $\SS$).
Applying \Lem{full},  either all edges in $\SS$ are colored $\rep(c)$ or 
there exists critical vertex $w \in \SS$ fully touched by $\rep(c)$ and another color.
In the former case, all edges connecting $\SS$ to $h$ are also colored $\rep(c)$
...
Consider some such $\rep(c)$, for an edge connecting to component $\SS$ of $\MM^+_v$.
Either all edges in $\SS$ are colored $\rep(c)$ or there exists critical vertex $w \in \SS$
fully touched by $\rep(c)$ and another color. In the latter case, $w$ has not been processed,
so $w \in T(\rep(c))$ (contradiction). Therefore, all edges in $\SS$ are colored $\rep(c)$.
Suppose there is some critical vertex in $\SS$. If it is not processed yet, it will
be in $T(\rep(c))$, which is not possible. Thus, all critical vertices in $\SS$ have been processed.

Consider $w$, the lowest critical vertex in $\SS$. When $w$ was processed, it was set to $\h(c')$, for
some color $c'$. At this stage, all edges in $\SS$ above $w$ were also colored $c'$ (just apply the argument
above for $w$). Since there are no critical points below $w$, all edges in $\SS$ below $w$
are also colored $c'$ (the painting property). This means that the edges connecting $\SS$
are also colored $c'$. So $c' = \rep(c)$, and $\h(\rep(c)) = w$ is a parent of $v$. 

This argument can be repeated for all such components $\SS$ to complete the proof.
\end{proof}
