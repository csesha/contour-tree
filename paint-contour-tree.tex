...
\begin{definition} \label{def:support} A light path $p$ is called \emph{supported} if: 
at least $|H_p|/100$ vertices in $p$ lie in heavy ancestors $q$ such that $|H_q| > |H_p|/100$.
%
%
% there exists
...
\begin{proof} (of \Lem{support}) We have $\sum_{p \in \cU} \sum_{v \in p} \log |H_v| = O((\log |H_r|) \sum_{p \in \cU} |p|) = O(|H_r|\log |H_r|)$.
...
This immediately proves part (i) of \Lem{cu-root}. Part (ii) requires some more work.
