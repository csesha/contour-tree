...
\begin{theorem} \label{thm:runtime} $\sum_{v \in T} \log |H_v| = O(\cost(\pmax(T)))$.
\end{theorem}
...
If $p$ is light, $\sum_{v\in p} \log |H_v| = O(|p| \log |H_p|)$.
...
\begin{definition} \label{def:support} A light path $p$ is called \emph{supported} if there exists
...
\begin{claim} \label{clm:above} $\sum_{p \in \cL'} |H_p| \log |H_p| = O(\sum_{p \in \cH} |p|\log |p|)$.
\end{claim}

\begin{proof} Pick $p \in \cL'$. As we traverse up $q_1, q_2, \ldots, q_k$, there
are certain vertices of $H_p$ that lie in these paths. There must be at least $|H_p|/100$
such vertices, since $H_{q_k} \leq 99H_p/100$. These are encountered in
a fixed order. Let $H'_p$ be the first half of these vertices. For all $v \in H'_p$
$|H_v| \geq |H_p|/200$. Hence, $|H_p|\log |H_p| = O(\sum_{v \in H'_p} \log |H_v|)$
and $\sum_{p \in L'} |H_p|\log |H_p| = O(\sum_{p \in L'} \sum_{v \in H'_p} \log |H_v|)$.
...
can only occur in one other $H'_q$ (for $q \in \cL'$), because $v$ has exactly two colors.
In other words, any $v$ appears at most twice in the above double summation.
These vertices are all present in heavy paths, so we can bound
the double summation by $\sum_{p \in \cH} \sum_{v \in p} \log |H_v|$.
%
...
Our focus is on $\pmax(T)$.
We can simplify a little. We split the summation and apply \Lem{pathBounds}. Finally,
we note that for $p \in \cL$, $|p| \leq |H_p|$.
\begin{eqnarray*} 
\sum_{p \in \cL} \sum_{v \in p} \log |H_v| & = & \sum_{p \in \cL \setminus \cL'} \sum_{v \in p} \log |H_v| + \sum_{p \in \cL'} \sum_{v \in p} \log |H_v|\\
...
(If not, we have reached the root $r$ of $\pmax(T)$. But $|H_r| = 0$, implying $p$ is supported.)
Insert edge from $p$ to $q$, so $q$ is the parent of $p$ in $\cU$. This construction leads
the shrub forest of $\cL \setminus \cL'$, where every unsupported light path is an internal node, and all
roots are supported light paths. \Lem{support} is a direct consequence of the following,
...
\begin{lemma} \label{lem:cu-root} (i) For any $v \in p$ such that $p \in \cU$,
$|H_v| = O(|H_r|)$. (ii) $\sum_{p \in \cU} |p| = O(|H_r|)$.
\end{lemma}

\begin{proof} (of \Lem{cu}) We have $\sum_{p \in \cU} \sum_{v \in p} \log |H_v| = O((\log |H_r|) \sum_{p \in \cU} |p|) = O(|H_r|\log |H_r|)$.
\end{proof}



\subsection{The root is everything in $\cU$} \label{sec:weight}
...
By construction, there is a heavy path (denoted $h(q)$) that is a child of $p$ in $\pmax(T)$,
and an ancestor of $q$. Furthermore, $|H_{h(q)}| > 99|H_q|/100$.
...
\end{lemma}
\begin{proof} We know that $h(q)$ is a child of $p$ in $\pmax(T)$. Consider the base
