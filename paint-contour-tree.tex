...
requires this time in the worst case, and our algorithm matches this bound. 
...
\begin{definition} \label{def:tree} The contour tree is the graph on vertex set $\cV= \cV(f)$, where
...
In particular, two $(f(x)+\delta)$-contours in $f|_{B_\eps(x)}$, given by \Def{deg}, might actually be the same contour in $\MM$. 
