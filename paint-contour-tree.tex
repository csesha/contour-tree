...
\begin{lemma} \label{lem:light} $\sum_{p \in \cL} \sum_{v \in p} \log |H_v| = O(\sum_{p \in \cL'} |H_p| \log |H_p|)$.
\end{lemma}

With this lemma, the proof of \Thm{runtime} is straightforward.

\begin{proof} (of \Thm{runtime}) We split into heavy and light paths.
...
\subsection{Proving \Lem{light}}

We can simplify a little. We split the summation and apply \Lem{pathBounds}. Finally,
we note that for $p \in \cL$, $|p| \leq |H_p|$.
\begin{eqnarray*} 
\sum_{p \in \cL} \sum_{v \in p} \log |H_v| & = & \sum_{p \in \cL'} \sum_{v \in p} \log |H_v| + \sum_{p \in \cL \setminus \cL'} \sum_{v \in p} \log |H_v| \\
& = & \sum_{p \in \cL \setminus \cL'} \sum_{v \in p} \log |H_v| + O(|p|\log |H_p|) = \sum_{p \in \cL \setminus \cL'} \sum_{v \in p} \log |H_v| + O(|H_p|\log |H_p|)
\end{eqnarray*}
...
\begin{lemma}
\label{lem:geometric}
 Let $p$ be the parent of $q$ in the shrub $\pmax(T)$.  
 Then $|H_q|\leq \frac{3}{5} |H_p|$. 
\end{lemma}
\begin{proof}
