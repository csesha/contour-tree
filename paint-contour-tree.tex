...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\InNotSoCGVer{\proofofRunTime}

%\InNotSoCGVer{\algorithmRunTimeSection}
%%%%%%%%%%%%%%%%%%%%
\InSoCGVer{
\subsection{Upper Bounds for Running Time}\label{sec:runTimeSummary}

% The full running time analysis can be found in \Sec{runTime}, \Sec{pathDecomp}, and \Sec{lb}, or in the full version \CiteFullVer.  
% \Sec{runTime} is summarized here, which gives a bound immediately implying the running time of our main result \Thm{main-corr}.  
% Using a significantly more sophisticated analysis involving path decompositions, this bound will also implies the more refined running time bounds mentioned in \Sec{more-refined} 
% are the topics \Sec{pathDecomp} and \Sec{lb}.
% %
...
%The heap as defined above characterizes the priority queue size at the time a vertex gets processed and hence we get the following.
...
Given the above technical definition, the proof of the following lemma is straightforward, though long and so has been moved to \Sec{runTime}.

\begin{lemma}\label{heapRunTimeBound}
...
Our main result, \Thm{main-corr}, is an easy corollary of the above lemma. Specifically, consider a critical point $v$ of the initial input 
...
denote the length of the longest directed path passing through $v$ in the contour tree the input complex, and let $\ell_v'$ 
denote the longest directed path passing through $v$ in the join tree of the piece containing $v$.
By \Thm{surgery}, ignoring non-dominant extremum introduced from cutting, the join tree on each piece output by $\rain$ is isomorphic to some 
...
and requires a significantly more sophisticated analysis involving path decompositions, which is the subject of \Sec{pathDecomp} and \Sec{lb}.
...
Note that form of the theorem statements in this section differ from \Thm{main-lb}, 
...
\section{Proof of Lemma~\ref{heapRunTimeBound}}
\label{sec:runTime}
\proofofRunTime
