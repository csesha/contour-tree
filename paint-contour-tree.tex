...
We will work with the \emph{reduced} heap $\redH_p$. This is the subset of $H_p$
of vertices that only appear in light paths. By definition, for any unsupported light path
(hence, any non-root $p \in \cU$), $|H'_p| \geq 99|H_p|/100$. For notational convenience, we simply
set $H'_r = H_r$.
...
\begin{lemma}
\label{lem:geometric}
 Let $p$ be the parent of $q$ in the shrub $\cU$. Then $|\redH_p|\geq \frac{3}{2}|\redH_q|$. 
\end{lemma}
...
Since $q$ is unsupported, these paths contain at most $|H_p|/100$ vertices of $H_p$. Thus, $|H_{h(q)}| \geq 99|H_p|/100$.
...
to get $|H_p| \geq 197|H_q|/100$. The proof is completed by noting that $|\redH_p| \geq 99|H_p|/100$ and $|H_q| \geq |\redH_q|$.
%
%
...
$\ell$ of $T$ in one of the ends of $p$. The reduced heap $\redH_p$ is the
...
\begin{claim} \label{clm:loss} $|\redH_p| \geq W_p - 2\sum_{q \in \cU_p} |q|$
\end{claim}
...
Other than $q$, all other nodes encountered are heavy paths and hence
do not affect reduced heaps.
The vertices of $\redH_{q'_i}$ that are deleted
are exactly those present in the path $q$ (which are paths in $T$) traversed. 
Any vertex in $q$ can be deleted from at most two reduced heaps $\redH_{q'_i}$.
When $R_q$ is negative, it is at most $2|q|$.
