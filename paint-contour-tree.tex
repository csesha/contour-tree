...
%For every vertex in $\cJ(\MM)$, let $H_v$ be defined by the initial coloring of $\MM$.
%The running time of $\build(\MM)$ is $O(N+t\alpha(t) + \sum_{v \in \cJ(\MM)} \log |H_v|)$.
\end{lemma}
...
% a function of some structural property of $\cJ(\MM)$.
...
% However, for the specific when $T=\cJ(\MM)$, plugging this theorem into the above lemma, and applying this lemma 
...
In this section, we set up a framework to analyze the time taken to compute a join tree $\cJ(\MM)$ (see \Def{criticalJoin}).
...
\InSoCGVer{
\begin{align*}
\hspace{.8in} O\pth{N+ \sum_{i=1}^k \pth{\sum_{p \in \pmax(\jc(\MM_i))} |p|\log |p|} + t_i\alpha(t_i) + N_i}& \\
=\break O\pth{\pth{\sum_{p \in \pmax(\cC(\MM))} |p|\log |p|} + t\alpha(t) + N}&
\end{align*}
}
\InNotSoCGVer{
\[
O\pth{N+ \sum_{i=1}^k \pth{\sum_{p \in \pmax(\jc(\MM_i))} |p|\log |p|} + t_i\alpha(t_i) + N_i}
=\break O\pth{\pth{\sum_{p \in \pmax(\cC(\MM))} |p|\log |p|} + t\alpha(t) + N}
\]
}
