...
basically $O(\sum_{v \in C} \log \ell_v)$, where $\ell_v$ is the depth of $v$ in the contour tree.
This matches al existing upper bounds, but is a significant improvement when the contour tree is short and fat.
...
can be computed in $O(\sum_{v \in C(T)} \log \ell_v + t\alpha(t) + N)$ time. Moreover, this algorithm only
...
Note that \Thm{main-corr} is a direct corollary of this statement. For any $v$, $\ell_v$ is at most
the length of the path in $P(T)$ that contains $v$.
This bound is strictly stronger,
since for any balanced contour tree, the run time bound of \Thm{main-alg} is $O(t\alpha(t) + N)$, and $O(t)$
...
comparisons in the worst-case over this set. (All further details are 
in Appendix~\Sec{lb}.)
...
A \emph{regular} point has both Morse up-degree and down-degree $1$. A \emph{maximum} has Morse up-degree $1$, while
a \emph{minimum} has Morse up-degree $2$. A \emph{Morse Join} has Morse up-degree strictly greater than $1$,
...
the wet part, and recurse of the dry part. This is carefully done to ensure that the total complexity
