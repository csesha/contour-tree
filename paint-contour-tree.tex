...
\begin{abstract}
Consider a scalar field $f:\MM \mapsto \RR$, where $\MM$ is a triangulated simplicial mesh in $\RR^d$. 
...
always bounded by $O(t\log t + N)$ and is $O(t\alpha(t)+N)$ for balanced trees. 
Moreover, we prove a strong optimality property of our algorithm, showing that $\sum_{p\in P} |p|\log|p|$ complexity
is inherent to computing contour trees. Consider the family of instances where our algorithm
requires $C$ comparison. Any algorithm computing contour trees on this family requires
$\Omega(C)$ comparisons in the worst case.
...
split into new components, contours may acquire or lose handles. These are the relevant events where 
...
hence must pay $\Omega(t\log t)$ for all instances. Can we beat this sorting bound for some instances? Is there a more refined version of the complexity
of computing contour trees, and can we characterize what inputs are hard? Our main result gives an affirmative answer. First, we require some technical definitions.
...
has maximum degree $3$. There is a deterministic algorithm whose running time is $O(\sum_{p \in P(T)} |p|\log |p| + t\alpha(t) + N)$,
...
A better but still obvious upper bound is $O(t\log D + t\alpha(t) + N)$, where $D$ is the diameter of $T$. 
This is clearly an improvement when the tree is short and fat.
But it can be even better. A calculation yields that for any decomposition of a rooted balanced binary tree, 
...
As referenced in \Thm{main-alg}, for any $f:\MM \mapsto \RR$ our deterministic algorithm defines a 
...
Consider the set of functions $\bF_C$ that all same number of critical points and the same (valid) cost $C$.  Any algebraic decision tree that correctly computes
...
Roughly speaking, it is not possible to improve on \Thm{main-alg} even when restricted to a subset
...
\begin{figure}[h]\centering
    \includegraphics[width=.28\linewidth]{figs/mound}%
    \hspace{.1in}
    \includegraphics[width=.28\linewidth]{figs/mound2}%
    \hspace{.1in}
    \includegraphics[width=.28\linewidth]{figs/mound3}%
    \hspace{.1in}
    ...
    On the far right, the contour tree.}
    \label{fig:colors}
\end{figure}
...
\Ben{This is already defined in section 3, should we use the same numbering?}
...
Specifically, proving the above theorem will imply our main result, \Thm{main-alg}, which we now restate and prove.

\begin{theorem}
Consider a PL-Morse $f:\MM \mapsto \RR$, where the contour tree $T$
has maximum degree $3$. There is an algorithm whose running time is $O(\sum_{p \in P(T)} |p|\log |p| + t\alpha(t) + N)$,
where $P(T)$ is a specific path decomposition (constructed implicitly by the algorithm).
\end{theorem}
\begin{proof}
\Ben{Prove the theorem, and change the numbering on the theorem.}
\end{proof}
...
\Ben{Hopefully the following sentence clear up confusion between $\MM$ and $(f,\MM)$}
Note earlier we usually used $\MM$ to denote the lift of a simplicial complex according to some height function $f$, 
however in this section we make a distinction between the complex $\MM$ and the height function $f$.  
In particular all previous definitions or subroutines which took $\MM$ as input now take $(f,\MM)$ as input.

\subsection{Join Trees}

In this section we prove \Thm{main-lb}, but for join trees instead of contour trees.  
%Observe that given a fixed $(f,\MM)$, 
%our algorithm for computing $\jc$ is deterministic and so the maximum path decomposition $P=P(\jc)$ defined by the initial coloring (\Def{initialColoring}) is fixed, 
%allowing us to define the cost, $\cost(f)=\sum_{p\in P} |p|\log|p|$.
%  
Let $\MM$ be fixed.  For a given height function $f$, we can now define $\cost_{\jc}(f)$.  Specifically, consider running the $\build(f,\MM)$ procedure 
of section \Sec{paint}, which computes the join tree $\jc(f,\MM)$.  This procedure calls $\init$, which will (uniquely) define some 
initial coloring of $\jc$ (\Def{initialColoring}).  Let $P(f)=P(\jc(f,\MM))$ be the maximum path decomposition of the join tree according to this coloring. 
Then $\cost_{\jc}(f) = \sum_{p\in P(f)} |p|\log|p|$
...
Let $\mathcal{P} = P(f)$ be the maximum path decomposition of some height function $f$ defined over a complex $\MM$. 
Then there is a family of functions, $\bF$, all defined over a common triangulation in plane, 
such that the size of $\bF$ is $\Pi_{p_i\in P} (|p_i|-1)!$, and no two function in $\bF$ define the same join tree.
\end{lemma}
\begin{proof}
 ...
 Now we describe how to attach two diamonds.  In this process, we glue the base of a scaled down ``child'' diamond on to a triangular cone face of the larger  ``parent'' diamond (see \Fig{diamond}).
 ...
 Let the set of height functions for these instances be denote $\bF$.
...
In the following we say to path decompositions $P_1$ and $P_2$ (of potentially different complexes and/or height functions) are equivalent 
if there is an graph isomorphism, $f$, between their respective shrubs, $S_1$ and $S_2$, which has the property that for any node $v\in S_1$ corresponding to a path of 
...
\begin{lemma}
\label{lem:cost}
Let $\mathcal{P}$ and $\bF$ be as defined above.  Then for all $f\in \bF$, $\cost_{\jc}(f) = \sum_{p\in P(f)} |p|\log|p| = \sum_{p\in \mathcal{P}} |p|\log |p|$
\end{lemma}
\begin{proof}
First observe that by construction, regardless of the choice of $f\in \bF$, there is a path decomposition $\overline{P}(T)$ of the join tree which is equivalent to $\mathcal{P}$.
...
We now show that regardless of how paint spilling is done, this will be the maximum path decomposition of $T$ (implying the lemma statement).

 So consider any order of paint spilling over the surface defined by $f$.
 Consider any join on a face of some diamond.  This join is the anchor point of some connecting child diamond.  
 The claim is that the two colors of this join (according to \Def{initialColoring})
...
Let $\mathcal{P} = P(f)$ be the rain consistent maximum path decomposition of some height function $f$ defined over a complex $\MM$. 
Then there is a family of functions, $\bF$, all defined over a common triangulation in plane, 
such that the size of $\bF$ is $\Pi_{p_i\in P} (|p_i|-1)!$, and no two function in $\bF$ define the same contour tree.
\end{lemma}
\begin{proof}
 As $\mathcal{P}$ is rain consistent, the paths can be partitioned into sets $P_1, \dots, P_k$, where $P_i$ is the set of all paths with 
...
% \end{figure}
 ...
 whether the BFS which produced it was an upward or downward BFS, respectively.  As the construction in \Lem{pathDecomp} was for join trees, for each terrain 
...
\end{figure} 
 ...
 on each diamond produces a surface with a distinct join tree.  The specific bound on the size of $\bF$, defining these distinct join trees, follows by applying the 
...
\begin{lemma}
Let $\mathcal{P}$ and $\bF$ be as defined above.  Then for all $f\in \bF$, $\cost(f) = \sum_{p\in P(f)} |p|\log|p| = \sum_{p\in \mathcal{P}} |p|\log |p|$
\end{lemma}
\begin{proof}
 This lemma follows immediately from \Lem{cost}.  Specifically, the sum $\cost(P)$, for any rain consistent maximum path decomposition $P$, can be broken 
 into the sums $\cost_{\jc}(P_i)$ of some maximum path decompositions $P_1, \dots P_k$.  Now apply \Lem{cost} to each of the $P_i$ separately and take the sum.
\end{proof}
