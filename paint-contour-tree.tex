...
We now define a weight function $w(v)$ for all $v\in \mathcal{L}$.  Such that $w(v)=\Theta(|H_v|)$.  
For this alternative quantity we will be able to show BLAH BLAH BLAH.

%Our goal is to use \Lem{geometric}  to show that $\sum_{v\in \mathcal{L}} |H_v| = O(|H_r|)$, namely 
%the total sum of the sizes of the heaps at the roots of all paths in the light path tree is on the 
%order of the size of the heap at the root of $r$.  ADD THE ARGUMENT saying how this fact will be used.
...
Then $\sum_{u\in \mathcal{P}_v} \sqrt{w(u)}/100 \leq \sum_{c\in C_v} \sqrt{w(c)}/20$
\end{lemma}
...
By assumption from the lemma statement we have $w(u_{i+1})\leq (3/5) w(u_i)$.  Therefore by the above calculation, 
...
\[
|H_v|\geq w(v)-\sum_{u\in \mathcal{L}_v} \sqrt{w(u)}/100 \geq w(v)- \sum_{c\in C_v} \sqrt{w(c)}/20 \geq \frac{4}{5} w(v)
\]
\end{claim}
...
we have $|H_v|\geq \frac{4}{5} w(v)$ and $w(v)\leq (4/5) w(p)$ for any two paths 
$x,y\in \mathcal{L}_v$ such that $y$ is the parent of $x$.
...
we have that $w(x)\leq (4/5) w(y)$ and so by \Lem{geometric}, 
